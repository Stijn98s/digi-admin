        ...
        <v-flex xs12>
            <v-data-table :headers="headers" :items="items" hide-actions :pagination.sync="pagination">
                ...
                <template slot="items" slot-scope="{ item }">
                    <td>{{ item.name }}</td>
                    <td>
                        <v-btn slot="activator" color="success" icon :to="{ path: '/users/' + item.name }">
            ...
            <v-dialog
                ...
                <v-card>
                    <v-card-text>
                        Weet je zeker dat je het account met de naam: <b>{{tempName}}</b> wilt verwijderen?
                    </v-card-text>
                    <v-card-actions>
                        <v-spacer></v-spacer>
...
<script>
    import { UserApiClient, User } from './../../plugins/api-client';
    ...
    export default {
        ...
        methods: {
            async get() {
                const elements = await UserApiClient().usersGet();
            ...
            async remove(id) {
                await UserApiClient().usersIdDelete(id);
                this.get();
                this.dialog= false;
            },

            async onPageChanged() {
                const elements = await UserApiClient().usersGet(this.pagination.current);
                this.items = elements.data;
            },
